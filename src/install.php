...
if (!checkCol($this->db, 'saferoute_delivery_company'))
    $this->db->query("ALTER TABLE `" . DB_PREFIX . "order` ADD `saferoute_delivery_company` VARCHAR(255) NOT NULL");

if (!checkCol($this->db, 'saferoute_delivery_type'))
    $this->db->query("ALTER TABLE `" . DB_PREFIX . "order` ADD `saferoute_delivery_type` TINYINT(1) DEFAULT 0");
